	...
	public:
		...
		static void sort(vector<T>& vec){
            ...
            for(auto it : vec){
				set.insert(it);
				count++;                
				if (count % (PROCESSBAR) == 0)
				{
					std::cout << "\r processing : "
							  << "[" << (count / PROCESSBAR) << "/" << (vec.size() / PROCESSBAR) << "]" << std::flush;
				}
			}
			std::cout << std::endl;
            uint64_t t = 0;
	...
	public:
		...
		static bool Debug(int seed) {
			...
			for (auto it : mapper.map) {
				if (p >= mapper.getLabel(it.second)) throw "error";
				if (a > it.first) throw "error";
				a = it.first;
		...
		static bool Test2(int num) {

			ListOrderMap<int, ListOrderMaintenanceTest> listOrderMap;
			for (LOInt i = 0; i < num; i++) {
				listOrderMap.add(i);
			}
			
			LOInt p = 0;
			for (auto it : listOrderMap.map) {
				if (p >= listOrderMap.getLabel(it.second)) throw "error";
