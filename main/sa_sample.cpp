...
int main(){
    ...
    std::vector<uint64_t> sa1 = stool::LO::OnlineBackwardSuffixSort<char>::construct_suffix_array(vec1);
    print<char>(vec1, sa1);

    std::cout << "uint8_t vector" << std::endl;
    std::vector<uint8_t> vec2{'m', 'i', 's', 'i', 's', 's', 'i', 'p', 'p', 'i'};
    std::vector<uint64_t> sa2 = stool::LO::OnlineBackwardSuffixSort<uint8_t>::construct_suffix_array(vec2);
    print<uint8_t>(vec2, sa2);


    std::cout << "int32_t vector" << std::endl;
    std::vector<int32_t> vec3{0, 123, -12, 12, -2, 2, 3, 5, 12, 12, -13};
    std::vector<uint64_t> sa3 = stool::LO::OnlineBackwardSuffixSort<int32_t>::construct_suffix_array(vec3);
    print<int32_t>(vec3, sa3);

    std::cout << "uint32_t vector" << std::endl;
    std::vector<uint32_t> vec4{12, 123, 12, 12, 2, 2, 3, 5, 12, 12, 13};
    std::vector<uint64_t> sa4 = stool::LO::OnlineBackwardSuffixSort<uint32_t>::construct_suffix_array(vec4);
    print<uint32_t>(vec4, sa4);

    std::cout << "int64_t vector" << std::endl;
    std::vector<int64_t> vec5{-12, 123, -12, -12, -2, -2, -3, -5, -12, -12, 13};
    std::vector<uint64_t> sa5 = stool::LO::OnlineBackwardSuffixSort<int64_t>::construct_suffix_array(vec5);
    print<int64_t>(vec5, sa5);

    std::cout << "uint64_t vector" << std::endl;
    std::vector<uint64_t> vec6{12, 123, 12, 2, 2, 2, 12, 2, 2, 3, 5, 12, 12, 13};
    std::vector<uint64_t> sa6 = stool::LO::OnlineBackwardSuffixSort<uint64_t>::construct_suffix_array(vec6);
